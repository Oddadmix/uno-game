...
class CardService {
  ...
  private readonly cardColors: CardColors[] = ['green', 'red', 'yellow'];

  async setupRandomCards(): Promise<CardData[]> {
    const randomCards: CardData[] = [...(await this.getCardStack()), ...(await this.getCardStack())];
  ...
  async retrieveRandomCardColor(): Promise<CardColors> {
    const cardColors: Array<CardColors> = ['green', 'yellow', 'red'];
  ...
  private buildCardPictureSrc(cardType: CardTypes, cardColor: CardColors) {
    ...
    const picturePath = `screw-driver/${cardType}/${cardColor}.jpeg`;
