...
import { Grid, ClickAwayListener, Button, Typography, Zoom } from '@material-ui/core';
...
const DraggableCard: React.FC<CardProps> = (props) => {
  ...
  return (
    <img
      ...
      className={className}
      alt={card.name}
      src={card?.show ? card.src : 'http://localhost:5000/assets/screw-driver/back.jpg'}
      style={{
        ...style,
...
const CardDeck: React.FC<CardDeckProps> = (props) => {
  ...
  const { isDraggingAnyCard } = useDragLayer((monitor) => ({
  ...
  const canBePartOfCurrentCombo = (cardType: CardTypes) =>
    !!cardStore?.selectedCards?.some((card) => card.type === cardType);
  ...
  const toggleSelectedCard = (cardId: string) => {
    ...
    if (isAlreadySelected) {
      ...
      cardStore.setSelectedCards([...(lastSelectedCards || []), selectedCard as CardData]);
    }
  };
  ...
  return (
    <ClickAwayListener onClickAway={handleClickOutsideCardDeck}>
      <Grid container alignItems='flex-end' justify='center' className={classes.cardContainer}>
        <PlayerEffect playerId={player?.id} />

        <Zoom in={player?.status === 'afk'}>
          <Grid container className={classes.afkContainer}>
            <Grid container alignItems='center' justify='center' direction='column' className={classes.afkContent}>
              <Typography variant='body1' className={classes.afkInfo}>
                We noticed you are afk, so we are making random plays automatically for you. In case you want to keep
                playing by yourself, click on the button below.
              </Typography>

              <Divider orientation='horizontal' size={2} />

              <Button variant='contained' className={classes.afkButton} onClick={toggleOnlineStatus}>
                I'M HERE
              </Button>
            </Grid>
          </Grid>
        ...
        >
          ...
          <Grid item>
            <Typography variant='h3' className={`${classes.title} ${customClasses.limitedName}`}>
              {player?.name}
            </Typography>
